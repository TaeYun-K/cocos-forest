...
public interface TreeRepository extends JpaRepository<Tree, Long> {
     ...
     * 특정 숲의 모든 나무 조회
     */
    List<Tree> findByForestIdOrderByXAscYAsc(Long forestId);

    /**
     * 특정 숲의 살아있는 나무만 조회
     */
    List<Tree> findByForestIdAndIsDeadFalseOrderByXAscYAsc(Long forestId);

    /**
     * 특정 위치의 나무 조회
     */
    Optional<Tree> findByForestIdAndXAndY(Long forestId, Integer x, Integer y);
    ...
    @Query("SELECT t FROM Tree t " +
            ...
            "AND t.isDead = false")
    List<Tree> findFullyGrownTrees(@Param("growthStage") GrowthStage growthStage,
    ...
    @Query("SELECT t FROM Tree t " +
            ...
            "AND t.health >= 80 " +
            "AND t.isDead = false")
    List<Tree> findFullyGrownTreesByUserId(@Param("userId") Long userId);

    /**
     * 죽음 하이라이트가 표시된 나무들 조회
     */
    List<Tree> findByForestIdAndDeadHighlightTrue(Long forestId);

    /**
     * 배치 작업: 모든 살아있는 나무의 체력 감소
     */
    @Modifying
    @Query("UPDATE Tree t SET t.health = GREATEST(t.health - :amount, 0), t.updatedAt = CURRENT_TIMESTAMP " +
     ...
     */
    @Modifying
    @Query("UPDATE Tree t SET t.isDead = true, t.deadHighlight = true, t.updatedAt = CURRENT_TIMESTAMP " +
     ...
     */
    @Modifying
    @Query("UPDATE Tree t SET t.growthDays = t.growthDays + 1, t.updatedAt = CURRENT_TIMESTAMP " +
            "WHERE t.isDead = false " +
     ...
     */
    @Modifying
    @Query("UPDATE Tree t SET t.growthDays = 0, t.updatedAt = CURRENT_TIMESTAMP " +
            "WHERE t.isDead = false " +
            "AND t.growthDays > 0 " +
     ...
     */
    @Modifying
    @Query("UPDATE Tree t SET t.waterCountToday = 0, t.updatedAt = CURRENT_TIMESTAMP")
